...
#SBATCH --gres=gpu:2
#SBATCH --job-name=tune_proj_SC_demo
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_proj_SC_demo_%j.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_proj_SC_demo_%j.err
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      ...
      --config models/configs/CrossModal_PCA_PLS_CovProjector_SC_demo.yml \
      --save_checkpoint \
      --use_tune \
      --source SC \
      --target FC \
