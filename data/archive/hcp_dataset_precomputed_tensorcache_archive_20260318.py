...
class HCP_Base():
    ...
    cov_sources=None):
        ...
        self.volume_feature_type = volume_feature_type
        self.centroid_feature_type = centroid_feature_type
        ...
        if self.data_load_mode not in {"manual", "precomputed"}:
            raise ValueError(
                f"Unknown data_load_mode='{self.data_load_mode}'. "
                "Choose from {'manual', 'precomputed'}."
            )
        ...
        # Load core modalities either by raw/manual path (with optional cache write)
        # or directly from precomputed npy caches.
        if self.data_load_mode == "manual":
            self.fc_subject_ids, self.fc_matrices, self.fc_upper_triangles = load_fc(
                parcellation,
                hemi,
                HCP_dir,
                write_cache=self.write_manual_cache,
                cache_root=self.precompute_cache_root,
            )
            self.sc_subject_ids, self.sc_matrices, self.sc_upper_triangles, self.sc_r2t_matrices = load_sc(
                parcellation,
                hemi,
                sc_metric_type,
                HCP_dir,
            ...
            (
                self.nodefeat_subject_ids,
                self.parcel_volume,
                self.parcel_centroids,
                self.parcel_node_features,
            ) = load_parcel_volume_centroids(
                parcellation=parcellation,
                hemi=hemi,
                HCP_dir=HCP_dir,
                volume_feature_type=volume_feature_type,
                centroid_feature_type=centroid_feature_type,
                write_cache=self.write_manual_cache,
                cache_root=self.precompute_cache_root,
            )
        else:
            self.fc_subject_ids, self.fc_matrices, self.fc_upper_triangles = load_fc_precomputed(
                parcellation=parcellation,
                hemi=hemi,
                cache_root=self.precompute_cache_root,
            )
            self.sc_subject_ids, self.sc_matrices, self.sc_upper_triangles, self.sc_r2t_matrices = load_sc_precomputed(
                parcellation=parcellation,
                hemi=hemi,
                metric_type=sc_metric_type,
                apply_log1p=sc_apply_log1p,
                cache_root=self.precompute_cache_root,
            )
            (
                self.nodefeat_subject_ids,
                ...
                self.parcel_centroids,
                self.parcel_node_features,
            ) = load_parcel_volume_centroids_precomputed(
                parcellation=parcellation,
                hemi=hemi,
                volume_feature_type=volume_feature_type,
                centroid_feature_type=centroid_feature_type,
        ...
        canonical_nodefeat_indices = self.subject_indices_from_id(self.nodefeat_subject_ids, canonical_subject_ids)
        ...
        if self.sc_r2t_matrices is not None:
            if self.sc_r2t_matrices.shape[0] != self.parcel_node_features.shape[0]:
                raise ValueError(
                    "Subject dimension mismatch between parcel node features and sc_r2t_matrices: "
                    f"{self.parcel_node_features.shape[0]} vs {self.sc_r2t_matrices.shape[0]}"
                )
            if self.sc_r2t_matrices.shape[1] != self.parcel_node_features.shape[1]:
                raise ValueError(
                    "Node dimension mismatch between parcel node features and sc_r2t_matrices: "
                    f"{self.parcel_node_features.shape[1]} vs {self.sc_r2t_matrices.shape[1]}"
                )
            self.parcel_node_features = np.concatenate(
                [self.parcel_node_features, self.sc_r2t_matrices.astype(np.float32)],
                axis=-1,
            )
