...
# Optuna + ASHA over the full nonlinear MLP search space.
#SBATCH --gres=gpu:2
#SBATCH --job-name=tune_model_parallel_maskedmlp_nonlinear
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedmlp_nonlinear_%j.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedmlp_nonlinear_%j.err
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      ...
      --config models/configs/MaskedMLPPretrainer_nonlinear.yml \
      --save_checkpoint \
      --use_tune \
      --search_alg optuna \
      --source SC \
