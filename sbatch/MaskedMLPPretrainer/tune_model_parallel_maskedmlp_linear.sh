...
# Optuna + ASHA over the linear MLP search space (low_rank_dim sweeps full
# vs. bottleneck rank, including null = full rank).
#SBATCH --gres=gpu:2
#SBATCH --job-name=tune_model_parallel_maskedmlp_linear
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedmlp_linear_%j.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedmlp_linear_%j.err
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      ...
      --config models/configs/MaskedMLPPretrainer_linear.yml \
      --save_checkpoint \
      --use_tune \
      --search_alg optuna \
      --source SC \
