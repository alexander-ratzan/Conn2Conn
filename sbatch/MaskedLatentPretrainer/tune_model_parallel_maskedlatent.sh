...
# Optuna + ASHA over the attention pretrainer search space defined in the
# canonical MaskedLatentPretrainer.yml.
#SBATCH --gres=gpu:2
#SBATCH --job-name=tune_model_parallel_maskedlatent
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedlatent_%j.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_model_parallel_maskedlatent_%j.err
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      --mode prod \
      --model MaskedLatentPretrainer \
      --config models/configs/MaskedLatentPretrainer.yml \
      --save_checkpoint \
      --use_tune \
      --search_alg optuna \
      --source SC \
