#!/bin/bash
# Array: 10 seeds for NodalMLP dot-product probe decoder (NodalMLP_dot.yml) (0-9).
# Probe heads have far fewer params than the MLP decoder; per-trial wall ~ similar to SConly tune.
#SBATCH --nodes=1
...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_nodalmlp_dot_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_dot_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_dot_array_%A_%a.err
...
SEED=${SEEDS[$IDX]}
CONFIG="NodalMLP_dot.yml"

echo "Starting job ${SLURM_JOB_ID} (task ${SLURM_ARRAY_TASK_ID}) on $(hostname) at $(date)"
echo "Model=NodalMLP  Source=SC  Decoder=dot  Config=${CONFIG}  Seed=${SEED}"
