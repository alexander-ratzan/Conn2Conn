#!/bin/bash
# Array: 10 seeds for NodalMLP spectral (Laplacian-eigenvector) encoder with basis-invariant
# decoder search over {dot, diag_bilinear} (NodalMLP_spectral.yml) (0-9).
# Per-batch eigh on the SC matrix is the dominant added cost (~ms/subject, negligible).
#SBATCH --nodes=1
...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_nodalmlp_spectral_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_spectral_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_spectral_array_%A_%a.err
...
SEED=${SEEDS[$IDX]}
CONFIG="NodalMLP_spectral.yml"

echo "Starting job ${SLURM_JOB_ID} (task ${SLURM_ARRAY_TASK_ID}) on $(hostname) at $(date)"
echo "Model=NodalMLP  Source=SC  Encoder=spectral  Decoder=dot|diag_bilinear  Config=${CONFIG}  Seed=${SEED}"
