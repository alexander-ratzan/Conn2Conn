#!/bin/bash
# Array: 10 seeds for NodalMLP symmetrized bilinear probe decoder (NodalMLP_bilinear.yml) (0-9).
# Bilinear adds D^2 params; D depends on use_encoder + sc_pca_dim.
#SBATCH --nodes=1
...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_nodalmlp_bilinear_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_bilinear_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_bilinear_array_%A_%a.err
...
SEED=${SEEDS[$IDX]}
CONFIG="NodalMLP_bilinear.yml"

echo "Starting job ${SLURM_JOB_ID} (task ${SLURM_ARRAY_TASK_ID}) on $(hostname) at $(date)"
echo "Model=NodalMLP  Source=SC  Decoder=bilinear  Config=${CONFIG}  Seed=${SEED}"
