...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_nodalmlp_SConly_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_SConly_array_%A_%a.out
...
echo "Model=NodalMLP  Source=SC  Features=SC-row-only  Config=${CONFIG}  Seed=${SEED}"
