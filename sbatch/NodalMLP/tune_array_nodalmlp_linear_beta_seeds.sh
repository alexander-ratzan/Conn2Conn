#!/bin/bash
# Array: 10 seeds for NodalMLP linear-beta probe decoder (NodalMLP_linear_beta.yml) (0-9).
# Single nn.Linear over the symmetric edge featurization; decoder_symmetry IS searched.
#SBATCH --nodes=1
...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_nodalmlp_linear_beta_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_linear_beta_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalmlp_linear_beta_array_%A_%a.err
...
SEED=${SEEDS[$IDX]}
CONFIG="NodalMLP_linear_beta.yml"

echo "Starting job ${SLURM_JOB_ID} (task ${SLURM_ARRAY_TASK_ID}) on $(hostname) at $(date)"
echo "Model=NodalMLP  Source=SC  Decoder=linear_beta  Config=${CONFIG}  Seed=${SEED}"
