...
#SBATCH --mem=96GB
#SBATCH --gres=gpu:4
#SBATCH --job-name=tune_nodalgnn_SC
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalgnn_SC_%j.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_nodalgnn_SC_%j.err
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      --mode prod \
      --model NodalGNN \
      --config models/configs/NodalGNN.yml \
      --source SC \
      --target FC \
      --shuffle_seed 0 \
