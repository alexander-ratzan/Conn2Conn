#!/bin/bash
# Array: 4 covariate conditions × 10 seeds = 40 tasks (0-39)
# task_id = seed_idx * 4 + cov_idx
#
# Conditions:
#   0: fs_all       — FreeSurfer all ROIs only
#   1: fs_volumes   — FreeSurfer volumes only
#   2: demo         — age + sex + race_eth only
#   3: fs_all_demo  — fs_all + age + sex + race_eth (combined)
#
#SBATCH --nodes=1
...
#SBATCH --mem=64GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_proj_cov_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_proj_cov_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_proj_cov_array_%A_%a.err
...
CONFIGS=(
  "CrossModal_PCA_PLS_CovProjector_SC_fs_all.yml"
  "CrossModal_PCA_PLS_CovProjector_SC_fs_volumes.yml"
  "CrossModal_PCA_PLS_CovProjector_SC_demo.yml"
  "CrossModal_PCA_PLS_CovProjector_SC_fs_all_demo.yml"
)
NUM_COVS=4
