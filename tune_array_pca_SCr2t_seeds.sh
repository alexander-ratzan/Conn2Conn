#!/bin/bash
# Array: 10 seeds for CrossModalPCA with SC_r2t source (0-9)
#SBATCH --nodes=1
...
#SBATCH --mem=32GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=tune_pca_SCr2t_array
#SBATCH --output=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_pca_SCr2t_array_%A_%a.out
#SBATCH --error=/scratch/asr655/neuroinformatics/Conn2Conn/results/logs/tune_pca_SCr2t_array_%A_%a.err
...
echo "Model=CrossModalPCA  Source=SC_r2t  Seed=${SEED}"
...
singularity exec --nv \
  ...
  /bin/bash -lc "
    ...
    python main.py \
      --mode prod \
      --model CrossModalPCA \
      --config models/configs/CrossModalPCA.yml \
      --source SC_r2t \
      --target FC \
      --shuffle_seed ${SEED} \
